...
interface CloudinaryState {
  ...
  isUploading: boolean
  error: string | null
  uploadToCloudinary: (file: File, resourceType: "video" | "image") => Promise<any>
  clearError: () => void
}
  ...
  uploadToCloudinary: async (file: File, resourceType: "video" | "image") => {
    ...
    try {
      ...
      const uploadResult = await new Promise((resolve, reject) => {
        ...
        xhr.open("POST", `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api"}/cloudinary/upload`)

...
router.get(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const analytics = {
        totalViews:
          publishedVideos.length * Math.floor(Math.random() * 10000) + 1000,
        totalLikes:
          publishedVideos.length * Math.floor(Math.random() * 500) + 50,
        totalComments:
          publishedVideos.length * Math.floor(Math.random() * 100) + 10,
        avgWatchTime: "3:45",
